...
>
> 如果您是离线导入本地镜像，请显式指定本地 tag。例如本地导入的是 `v0.4.1-pidfix`：
> `helm install etmem-operator ./deploy/helm/ -n etmem-system --create-namespace --set operator.image=etmem-operator:v0.4.1-pidfix --set agent.image=etmem-agent:v0.4.1-pidfix`
>
> 标准交付入口是 **Helm Chart + 版本化镜像 tag**；离线 tar 包只是镜像分发介质，用于 `nerdctl load` 之后再执行 Helm 安装或升级。
